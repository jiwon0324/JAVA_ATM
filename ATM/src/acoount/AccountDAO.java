...
public class AccountDAO {
	...
	public void addAccountsData(String accData) {
		list = new ArrayList<Account>();
	...
	public void addAccount(int clientNo, ClientDAO clientDAO) {
		...
		String accNum = utils.getValue("계좌번호를 입력:");
	...
	public void deposit(int clientNo) {
		String accNum = utils.getValue("입금할 계좌번호:");
		int idx = AccountNumber(accNum, clientNo, "입금");
        if(idx == -1) return;
		int money = utils.getValue("입금할 금액을 :", 100, 1000000);
	...
	public void withdraw(int clientNo) {
		String accNum = utils.getValue("출금할 계좌번호:");
		int idx = AccountNumber(accNum, clientNo, "출금");
        if(idx == -1) return;
        
		utils.printMsg("현재 잔액은 " + list.get(idx).getMoney() + "원 ");

		int money =  AccountBalance("출금", idx);
	...
	private int AccountBalance(String state, int idx) {
		int money = 0;
		...
		while (true) {
			money = utils.getValue( state +" 할 금액을 입력:", 100, list.get(idx).getMoney());
	...
	private int AccountNumber(String accNum, int clientNo, String state) {
		int idx = getAccIdxFromAccNum(accNum);
		if (idx == -1) {
			utils.printMsg("-존재하지 않습니다.-");
			return -1;
	...
	public void transfer(int clientNo) {
        ...
        int idx = AccountNumber(accNum, clientNo, "이체");
        if(idx == -1) return;
  
        utils.printMsg("현재 잔액은 " + list.get(idx).getMoney() + "원 입니다.");
    	int money =  AccountBalance("이체", idx);
    	if(money == -1) return;
    	
        utils.printMsg("이체할 계좌번호를 입력하세요");
        String transferAcc = utils.getValue("계좌번호:");
